...
class AI:
    ...
    def get_possible_endandered_areas(self, board):
        ...
        return sorted(endangered_areas, reverse=True, key=lambda endangered_area: endangered_area[1][0] - endangered_area[0][1])

    def gen_helping_defense_path(self, board, deffender, dice_count, ignore_areas, threat, transfers_left):
        def border_with_enemy(area):
        ...
        # Get possible helpers of deffender
        possible_helpers = board.get_area(deffender[0]).get_adjacent_areas_names()
        ...
        for possible_helper in possible_helpers:
            ...
            # Skip enemy
            if poss_helper_area.get_owner_name() != self.player_name:
                continue
            ...
            if border_with_enemy(poss_helper_area):
                continue
    ...
    def gen_deffense_moves(self, board, transfers_left):
        ...
        print("DEFENSE", len(endangered_areas))
        for endangered_area in endangered_areas:
            pprint(endangered_area)

        # Go through each endangered area, and find helping path
        for endangered_area in endangered_areas:
            # Find path
            path = self.gen_helping_defense_path(board, endangered_area[0], 1, [], endangered_area[1], transfers_left)
